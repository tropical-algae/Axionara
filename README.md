...
Axionara 是一个用于覆盖数据资产上传、治理审核、市场发布、授权使用及智能问答等环节的全栈平台。平台将数据提供者、平台管理员和数据使用者整合于统一的工作流之中，旨在打造全流程可控、安全可靠的数据要素一体化服务与管理系统。待交易数据经过解析、清洗与风险识别后，由管理员统一发布，提供给经过授权认证的平台使用者进行交易或导出。

平台内置数据分析模块，以及由 RAG 支持的 Agent 问答系统，支持使用者对各类数据进行交互式分析
