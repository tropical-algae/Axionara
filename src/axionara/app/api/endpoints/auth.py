...
async def user_register(
    ...
    existed_user = await select_user_by_username(db=db, username=user.username)
    if existed_user is not None:
        raise HTTPException(**CONSTANT.RESP_USER_EXISTS)

    existed_user = await select_user_by_email(db=db, email=user.email)
    if existed_user is not None:
        raise HTTPException(**CONSTANT.RESP_USER_EMAIL_EXISTS)

    new_user = await insert_user(db=db, user=user.build_user())
...
async def login_access_token(
    ...
    user = await select_user_by_login_name(db=db, login_name=form_data.username)
